...
namespace artest::engine
{
	...
	[[nodiscard]] nlohmann::json SerializeCompileResult(bool valid, std::size_t instrumentCount, std::size_t stepCount, const std::vector<artest::Diagnostic> &diagnostics);
	[[nodiscard]] nlohmann::json SerializeResult(const artest::RunResult &value, int schemaVersion = 1);
